...
# # def get_html_and_js_with_custom_user_agent(url, user_agent):
# #     try:
...
# #         # Extract the JavaScript from each script element
# #         javascript_code = [script.get_text() for script in script_tags]
...
# # def main():
# #     st.title("Beautify HTML, CSS, and JavaScript with User Agent and Extract Pattern")

# #     # Get the URLs from the user, separated by line breaks
# #     url_input = st.text_area("Enter the URLs of the webpages (one URL per line):")

# #     if st.button("Beautify and Extract Pattern"):
# #         # Split the input into separate lines and clean up any extra whitespace
# #         urls = [url.strip() for url in url_input.split('\n') if url.strip()]

# #         if not urls:
# #             st.warning("Please enter at least one valid URL.")
# #         else:
# #             # Select a user agent from the list (you can use random.choice() for random selection)
# #             selected_user_agent = user_agents[0]

# #             # Initialize a list to store extracted patterns from each URL
# #             extracted_patterns = []

# #             for url in urls:
# #                 html_content, _ = get_html_and_js_with_custom_user_agent(url, selected_user_agent)

# #                 if html_content:
# #                     st.header(f"Beautified HTML for URL: {url}")
# #                     # Beautify and display the HTML content
# #                     st.text_area(f"HTML Content for URL: {url}", html_content, key=f"html_content_{url}")

# #                     # Extract the specified pattern from the beautified HTML content
# #                     extracted_pattern = extract_pattern(html_content, "https://player.movi.pk/player/embed_player.php?vid=")
# #                     if extracted_pattern:
# #                         extracted_patterns.extend(extracted_pattern)

# #             if extracted_patterns:
# #                 st.header("Extracted Patterns:")
# #                 st.text_area("Extracted URLs", "\n".join(extracted_patterns))
# #             else:
# #                 st.info("Pattern not found in the HTML content of any URL.")
...
# @st.cache_data
# def load_data():
#     data = pd.read_csv(r"C:\Users\style\sample test 2 - Copy.csv")
#     return data

# movie_data = load_data()

# # Streamlit app
# st.title("Movie URL Search")
